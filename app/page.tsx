...
export default function ShippingManagementApp() {
  ...
  // 端末へローカル保存（ダウンロード誘導）する補助関数
  const saveToDevice = (file: File | Blob, fileName: string) => {
    try {
      ...
      console.error("端末保存失敗:", e);
    }
  };

  // ----------------------------------------------------
  // 複数画像自動圧縮＆即時プレビュー・端末保存・アップロード
  // ----------------------------------------------------
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // 端末ローカル保存（ダウンロードを自動発生）
        saveToDevice(compressed, `TMS_${seqNo}_${file.name}`);
  ...
  return (
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4 w-full box-border overscroll-y-contain touch-manipulation overflow-x-hidden">
          ...
          <div className="space-y-6 text-center py-6 w-full">
            ...
            <div className="p-4 bg-zinc-900/50 border border-zinc-800/50 rounded-xl space-y-2 text-left w-full box-border">
              ...
              <input
                ...
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-amber-500 box-border"
                onKeyDown={(e) => {
      ...
      {/* ======================================================== */}
      {/* 閉じるボタンを最前面かつ最上部に固定配置したモーダル     */}
      {/* ======================================================== */}
      {previewIndex !== null && images[previewIndex] && (
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col justify-between p-4 backdrop-blur-md">
          {/* ヘッダー（閉じるボタンを前面・高コントラスト化） */}
          <div className="flex justify-between items-center text-zinc-300 z-20 pb-2 border-b border-zinc-800/80">
          ...
          {/* ズーム領域 */}
          <div className="flex-1 w-full h-full my-2 overflow-hidden flex items-center justify-center relative z-10">
          ...
          {/* フッターナビ */}
          <div className="flex justify-between items-center gap-4 z-20 pt-2 border-t border-zinc-800/80">
            <button
